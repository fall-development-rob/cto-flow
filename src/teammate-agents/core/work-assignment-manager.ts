 ...
 * 5. Notify agent
 *
 * Graceful degradation: Returns null if teammate mode is off or no suitable agent found.
 */
...
import { TeammateConfigManager } from './config-manager';
...
export class WorkAssignmentManager {
   ...
   *
   * Workflow:
   * 1. Check if teammate mode is enabled
  ...
  public async assignWork(
    ...
    // Graceful degradation: Check if teammate mode is enabled
    if (!this.configManager.isTeammateModeEnabled()) {
      return null;
    }
