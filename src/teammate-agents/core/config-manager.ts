 ...
 *
 * Manages configuration for the Teammate-Driven Agent Management system.
...
/**
 * Complete configuration schema for Teammate mode
 */
 ...
 */
export const DEFAULT_CONFIG: TeammateConfig = {
  enabled: false,
  github: {
    owner: '',
    repo: '',
...
const ENV_VAR_MAPPINGS = {
  'CLAUDE_FLOW_TEAMMATE_MODE': 'enabled',
  'CLAUDE_FLOW_TEAMMATE_GITHUB_OWNER': 'github.owner',
  'CLAUDE_FLOW_TEAMMATE_GITHUB_REPO': 'github.repo',
  'CLAUDE_FLOW_TEAMMATE_SYNC_INTERVAL': 'github.syncInterval',
  'CLAUDE_FLOW_TEAMMATE_WEBHOOK_ENABLED': 'github.webhookEnabled',
  'CLAUDE_FLOW_TEAMMATE_EPIC_LABEL': 'github.epicLabel',
  'CLAUDE_FLOW_TEAMMATE_AUTO_ASSIGNMENT': 'agents.autoAssignment',
  'CLAUDE_FLOW_TEAMMATE_PEER_REVIEW': 'agents.peerReview',
  'CLAUDE_FLOW_TEAMMATE_CONTEXT_SHARING': 'agents.contextSharing',
  'CLAUDE_FLOW_TEAMMATE_ASSIGNMENT_THRESHOLD': 'agents.assignmentThreshold',
  'CLAUDE_FLOW_TEAMMATE_PERSIST_TO_EPIC': 'memory.persistToEpic',
  'CLAUDE_FLOW_TEAMMATE_RESTORE_FROM_EPIC': 'memory.restoreFromEpic',
  'CLAUDE_FLOW_TEAMMATE_MEMORY_TTL': 'memory.ttl',
  'CLAUDE_FLOW_TEAMMATE_NAMESPACE_PREFIX': 'memory.namespacePrefix'
};
 ...
 * for the Teammate-Driven Agent Management system.
 */
export class TeammateConfigManager {
  private static instance: TeammateConfigManager | null = null;
   ...
   */
  public static getInstance(): TeammateConfigManager {
    if (!TeammateConfigManager.instance) {
      TeammateConfigManager.instance = new TeammateConfigManager();
    }
    return TeammateConfigManager.instance;
  }
  ...
  public static resetInstance(): void {
    TeammateConfigManager.instance = null;
  }
   ...
   */
  public loadConfig(options: Partial<TeammateConfig> = {}): TeammateConfig {
    // Start with defaults
    this.config = this.deepClone(DEFAULT_CONFIG);
   ...
   */
  public getConfig(): TeammateConfig {
   ...
   */
  public updateConfig(updates: Partial<TeammateConfig>): TeammateConfig {
   ...
   */
  public isTeammateModeEnabled(): boolean {
  ...
  /**
   * Check if Teammate mode can be used (enabled and GitHub configured)
   */
  public canUseTeammateMode(): boolean {
    return this.isTeammateModeEnabled() && this.isGitHubConfigured();
  }
  ...
  public validateConfig(): ConfigValidationResult {
    ...
    if (this.config.enabled) {
      if (!this.config.github.owner || this.config.github.owner.trim() === '') {
        errors.push('github.owner is required when Teammate mode is enabled');
      }

      if (!this.config.github.repo || this.config.github.repo.trim() === '') {
        errors.push('github.repo is required when Teammate mode is enabled');
      }
    ...
    if (this.config.memory.restoreFromEpic && !this.config.enabled) {
      warnings.push('memory.restoreFromEpic is enabled but Teammate mode is disabled');
    }
   ...
   */
  private applyOverrides(overrides: Partial<TeammateConfig>): void {
...
/**
 * Utility function: Check if Teammate mode is enabled
 */
export function isTeammateModeEnabled(): boolean {
  return TeammateConfigManager.getInstance().isTeammateModeEnabled();
}
...
export function isGitHubConfigured(): boolean {
  return TeammateConfigManager.getInstance().isGitHubConfigured();
}

/**
 * Utility function: Check if Teammate mode can be used
 */
export function canUseTeammateMode(): boolean {
  return TeammateConfigManager.getInstance().canUseTeammateMode();
}
...
export function validateConfig(): ConfigValidationResult {
  return TeammateConfigManager.getInstance().validateConfig();
}

/**
 * Utility function: Get current configuration
 */
export function getConfig(): TeammateConfig {
  return TeammateConfigManager.getInstance().getConfig();
}

/**
 * Utility function: Load configuration with options
 */
export function loadConfig(options: Partial<TeammateConfig> = {}): TeammateConfig {
  return TeammateConfigManager.getInstance().loadConfig(options);
}

/**
 * Export the singleton instance getter for advanced usage
 */
export function getConfigManager(): TeammateConfigManager {
  return TeammateConfigManager.getInstance();
}
