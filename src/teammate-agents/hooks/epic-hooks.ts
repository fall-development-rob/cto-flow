/**
 * Epic Lifecycle Hooks for Teammate-Driven Agent Management
 *
 ...
 *
 * Conditional execution: Only runs when teammate mode is enabled
 */
  ...
  createHandler(): HookHandler {
    return async (payload: HookPayload, context: AgenticHookContext): Promise<HookHandlerResult> => {
      ...
      if (!canUseTeammateMode()) {
        logger.debug('Teammate mode disabled, skipping pre-epic hook');
        return {
          continue: true,
          modified: false,
        };
...
export class PostEpicPhaseHook {
  ...
  createHandler(): HookHandler {
    return async (payload: HookPayload, context: AgenticHookContext): Promise<HookHandlerResult> => {
      // Check if teammate mode is enabled
      if (!canUseTeammateMode()) {
        logger.debug('Teammate mode disabled, skipping post-epic-phase hook');
        return {
          continue: true,
          modified: false,
        };
  ...
  private async syncToGitHub(
    ...
    const config = getTeammateConfig();
...
export class PostSpecificationHook {
  ...
  createHandler(): HookHandler {
    return async (payload: HookPayload, context: AgenticHookContext): Promise<HookHandlerResult> => {
      // Check if teammate mode is enabled
      if (!canUseTeammateMode()) {
        logger.debug('Teammate mode disabled, skipping post-specification hook');
        return {
          continue: true,
          modified: false,
        };
 ...
 * to register all teammate-driven agent management hooks.
 *
 * All hooks check canUseTeammateMode() internally and gracefully
 * no-op when teammate mode is disabled.
 */
 ...
 *
 * Useful for testing or when disabling teammate mode at runtime
 */
