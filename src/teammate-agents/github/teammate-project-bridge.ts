 ...
 *
 * Bridges the TeammateManager with GitHub Projects for full lifecycle tracking.
 ...
 *
 * @module github/teammate-project-bridge
 */
...
export interface TeammateProjectConfig {
  github: {
    owner: string;
    repo: string;
    ownerType: 'user' | 'org';
...
export const DEFAULT_PROJECT_CONFIG: TeammateProjectConfig = {
  github: {
    owner: '',
    repo: '',
    ownerType: 'user'
...
export class TeammateProjectBridge extends EventEmitter {
  private config: TeammateProjectConfig;
  ...
  constructor(
    config: Partial<TeammateProjectConfig>,
  ...
  async assignAgentToIssue(
    ...
    // Store in memory
    await this.memoryManager.store(
      `teammate:assignment:${agentId}:${issueNumber}`,
      assignment,
...
export function createTeammateProjectBridge(
  config: Partial<TeammateProjectConfig>,
  memoryManager: IMemoryManager
): TeammateProjectBridge {
  return new TeammateProjectBridge(config, memoryManager);
}

export default TeammateProjectBridge;
