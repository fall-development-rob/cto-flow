/**
 * Teammate-Driven Agent Management - CLI Commands
 *
 * Comprehensive CLI implementation for epic management, context restoration,
 * and teammate mode control in the claude-flow framework.
 *
 * @module teammate-agents/cli/commands
 */
...
import { TeammateConfig } from '../core/config.js';
import {
  Epic,
  EpicState,
...
/**
 * Check if teammate mode is enabled from config or environment
 */
function isTeammateModeEnabled(overrideFlag?: boolean): boolean {
  if (overrideFlag !== undefined) {
    return overrideFlag;
  }

  const config = TeammateConfig.getInstance();
  return config.get('teammate.enabled', false);
}

/**
 * Show helpful message when teammate mode is disabled
 */
function showTeammateModeDisabledMessage(command: string): void {
  console.log(chalk.yellow('\n⚠️  Teammate Mode is currently disabled\n'));
...
export function createEpicCommand(): Command {
  ...
  epic
    ...
    .option('--teammate-mode', 'Enable teammate mode for this command')
    .option('--no-teammate-mode', 'Disable teammate mode for this command')
    .description('Create a new epic')
    .action(async (title: string, options: any) => {
      const teammateEnabled = isTeammateModeEnabled(
        options.teammateMode === true ? true :
        options.teammateMode === false ? false :
        undefined
      );

      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow epic create');
        return;
      }
  ...
  epic
    ...
    .action(async (options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow epic list');
        return;
      }
  ...
  epic
    ...
    .action(async (epicId: string, options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow epic show');
        return;
      }
  ...
  epic
    ...
    .action(async (epicId: string, options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow epic update');
        return;
      }
  ...
  epic
    ...
    .action(async (epicId: string, options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow epic sync');
        return;
      }
  ...
  epic
    ...
    .action(async (epicId: string, options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow epic assign');
        return;
      }
 ...
 */
export function createTeammateCommand(): Command {
  ...
  teammate
    ...
    .action(async (options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow teammate context-restore');
        return;
      }
  ...
  teammate
    ...
    .action(async (options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow teammate context-save');
        return;
      }
  ...
  teammate
    ...
    .action(async (options: any) => {
      ...
      if (!teammateEnabled) {
        showTeammateModeDisabledMessage('npx claude-flow teammate context-clear');
        return;
      }
...
export function registerTeammateCommands(program: Command): void {
  ...
  program.addCommand(createTeammateCommand());
}

/**
 * Parse and execute teammate commands
 */
export async function executeTeammateCommand(args: string[]): Promise<void> {
  const program = new Command();

  program
    .name('claude-flow')
    .description('Claude Flow with Teammate-Driven Agent Management')
    .version('2.7.47');
